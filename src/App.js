    ...
    (time) => {
      ...
      setData((data) => {
        ...
        if (plotData.current.length >= 1000) {
          ...
          return { ...data, alt: 0, throttle: 0, autoPilot: false, message: 'Log Full - View plot to reset', frameRate: frameRate.toFixed(1), error_sum: 0 };
        }

        return { ...data, alt, alt_d, alt_dd, error, error_sum, throttle: throttleRef.current, message: message, frameRate: frameRate.toFixed(1) };
      });
