...
function App() {
  ...
  return (
    <ChakraProvider theme={theme}>
      {/* Title Box */}
      <Box color={'white'} fontWeight={200} bg={'teal.400'} mx={10} my={4} borderRadius={'lg'} p={4} textAlign={'center'} >
        <Text fontSize='2xl'>
      ...
      <Flex w='100%' h='100%' justifyContent={'center'} alignItems={'center'}>
        ...
        <ThrottleBox throttle={data.throttle}
          setThrottle={(throttle) => {
            //setData({ ...data, throttle })
            throttleRef.current = throttle;
          }}
        />
        <FlightBox alt={data.alt} height={600} />
        <HudBox alt={data.alt * ceiling}></HudBox>
